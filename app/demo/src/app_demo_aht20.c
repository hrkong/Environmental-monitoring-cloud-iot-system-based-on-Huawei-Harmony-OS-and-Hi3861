...
extern hi_u32 sgp30_read(hi_u32 recv_len, hi_u8 type);
/*
...
hi_u32 aht20_read(hi_u32 recv_len, hi_u8 type)
{
    ...
    if (type == AHT_TEMPERATURE) {
        temper = (hi_float)((recv_data[3] &0x0f)<<16 | recv_data[4]<<8 |recv_data[5]);//温度拼接
    ...
    if (type == AHT_HUMIDITY) {
        humi = (hi_float)((recv_data[1]<<12 | recv_data[2]<<4) | ((recv_data[3] & 0xf0)>>4));//湿度拼接
...
hi_void app_demo_aht20_task(hi_void)
{
    ...
    attr.task_prio = AHT20_DEMO_TASK_PRIORITY;
    attr.task_name = (hi_char*)"app_demo_aht20_task";
